...
double SPH<D,DD>::dNdpdphi(double p, double phi, HAD cur)
{
	...
	out+=sub;  
        ...
        //Added or typ==4	 
	if ((typ==1)||(typ==3)||(typ==4)||(typ==5)) {double sub2=qv[i].x[0]*I1c+qp*I2c;//had 4
	...
	else if (typ==5) outc=vfac*out+cur.svfac*outsc; //added v2
...
void SPH<D,DD>::Iout(double &I1, double &I2, double p, double phi, HAD cur,int nsph)// Added to read mu's as well
{
	...
	double bfac=eperp/TG;
      ...
      //I added type==4 here
	if ((typ==1)||(typ==3)||(typ==4) || (typ ==5) ) calcF2(cur,nsph,pd,f0s,f1s,f2s);//had 4
	...
	{expT=0;}
    else {expT=exp((pd/par[nsph].T)-((cur.baryon*par[nsph].muB + cur.strange*par[nsph].muS+ cur.electric*par[nsph].muQ)/par[nsph].T));}				  
	...
	//add a routine down for type 4	
		if ((typ==1)||(typ==3)||(typ==4) || (type ==5))//had 4
		{
