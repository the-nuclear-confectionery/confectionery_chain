...
////	// old version ends here
	...
	,&par2[i].n.x[2],&par2[i].u.x[0],&par2[i].u.x[1],&par2[i].u.x[2],&par2[i].vol,&par2[i].pi00,&par2[i].pi11,&par2[i].pi22
	,&par2[i].pi33,&par2[i].pi12,&par2[i].tau,&par2[i].r.x[0],&par2[i].r.x[1],&par2[i].s,&par2[i].T); 
	par2[i].vol/=sc3;
	par2[i].nu=par2[i].n&par2[i].u;
	if (par2[i].nu<0)
	{
...
double SPH<D,DD>::dNdpdphi(double p, double phi, HAD cur)
{
	...
	else if (typ==3 ||(typ==5)) outc=vfac*outc+cur.svfac*outsc;
...
void SPH<D,DD>::Iout(double &I1, double &I2, double p, double phi, HAD cur,int nsph)// Added to read mu's as well
{
	...
	else if ((cur.id = 211 ) && (abs(par[nsph].muQ) > 0.11))
	{expT=0;} */
	...
	//-((cur.baryon*par[nsph].muB + cur.strange*par[nsph].muS+ cur.electric*par[nsph].muQ)/par[nsph].T));}
